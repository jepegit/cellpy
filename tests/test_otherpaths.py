...
def test_is_external(test_input, expected):
    p1 = cellpy.internals.connections.OtherPath(test_input)
    assert p1.is_external == expected
    p1 = cellpy.internals.connections.OtherPath(p1)
    assert p1.is_external == expected
    if test_input is not None:
        p2 = pathlib.Path(test_input)
        p3 = cellpy.internals.connections.OtherPath(p2)
        assert p3.is_external == expected
        p4 = cellpy.internals.connections.OtherPath(p3)
...
def test_wrap(parameters):
    p = cellpy.internals.connections.OtherPath(parameters.res_file_path)
    p.is_file()
    p.home()
...
def test_stat(parameters):
    ...
    p_local = cellpy.internals.connections.OtherPath(parameters.res_file_path)
...
def test_copy_remote_simple(
    ...
    remote_file_path = cellpy.internals.connections.OtherPath(p2)
