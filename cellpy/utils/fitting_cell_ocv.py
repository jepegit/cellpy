...
# http://stackoverflow.com/questions/11620914/removing-nan-values-from-an-array
    ...
    initial_param.add('r_ct', value=init_guess['r_rc']['ct'], min=0)
    initial_param.add('r_d', value=init_guess['r_rc']['d'], min=0)
    initial_param.add('c_ct', value=init_guess['c_rc']['ct'], min=0)
    initial_param.add('c_d', value=init_guess['c_rc']['d'], min=0)
    initial_param.add('ocv', value=init_guess['ocv'])
    initial_param.add('v_rlx', value=init_guess['v_rlx'], vary=False)
    # guessed_voltage = ocv_relax_func(time[0], init_guess[0]['ocv'],
    #                                  init_guess[0]['v_rlx'],
    #                                  init_guess[0]['r_rc'],
    #                                  init_guess[0]['c_rc'])
    # Fitting data
    Mini = Minimizer(ocv_user_adjust, params=initial_param,
                     fcn_args=(time[0], voltage[0])).minimize()
    result = [Mini]
    best_para = [result[0].params]
    best_fit_voltage = [result[0].residual + voltage[0]]
    report_fit(result[0])
    ...
    for cycle_i in range(1, len(time)):
        ...
        best_fit_voltage.append(result[cycle_i].residual + voltage[cycle_i])
