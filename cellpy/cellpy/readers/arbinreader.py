...
class dataset:
    ...
    def makeDataFrame(self):
        self.dfdata    = pd.DataFrame(self.data).sort([self.datapoint_txt])
            ...
            try:
                if args[0].verbose:
                    ...
                    if args[0].intendation == 1:
                        s_line = str(50*"*")
                        t_line = str(25*" -")
                    elif args[0].intendation >1 and args[0].intendation <12:
...
#        print
        ...
        if merge_step_table:
            ...
            if step_table_made:
                cycle_index_header = self.cycle_index_txt
                t2.step_table[step_table_txt_cycle] = t2.dfdata[step_table_txt_cycle] + last_cycle
...
#        minimum_ierror_limit = 0.0001 # A     
        ...
        mask_no_current_hard = (df_steps[max_current_txt].abs() + df_steps[min_current_txt].abs()) < current_limit_value_hard
        mask_no_current_soft = (df_steps[max_current_txt].abs() + df_steps[min_current_txt].abs()) < current_limit_value_soft
...
#                                   'rest','not_known']
        ...
        df_steps.loc[mask_voltage_stable & mask_current_negative & mask_current_down, step_table_txt_type] = 'cv_discharge'
...
#                    self.Print(e,1)
        ...
        if total_current<typicall_current_max and total_current>0.0:
            small_current = True
            ...
            if step_type.lower() == "ir":
                ...
                for j in range(1, no_cycles+1):
                    ...
                    for s in steps:
                        ...
                        if total_current<typicall_current_max and total_current>0:
                            small_current = True
                ...
                if len(step)>1:
                    for s in step:
                        ...
                        if first:
                            ...
                            selected=_selected.append(c, ignore_index=True)
                else:
                    c = self._select_step(cycle,step, test_number)
                    if first:
                        selected=c.copy()
...
#            self.Print(ocv_steps[cycle_number])
    ...
    def get_ir(self,
        ...
        d2 = d2[["Cycle_Index","DateTime","Data_Point","Internal_Resistance"]].sort([self.data_point_txt]) # jepe fix
...
#-----------internal-helpers---------------------------------------------------
    ...
    def _modify_cycle_number_using_cycle_step(self,from_tuple = [1,4], to_cycle = 44, test_number=None):
        ...
        st = self.tests[test_number].step_table  
        st[step_table_txt_cycle][ (st[step_table_txt_cycle] == from_tuple[0] ) & (st[step_table_txt_step] == from_tuple[1]) ] = to_cycle
        # modifying normal_table
        nt = self.tests[test_number].dfdata
        nt[cycle_index_header][ (nt[cycle_index_header] == from_tuple[0]) & (nt[step_index_header] == from_tuple[1])] = to_cycle
...
#                    print cm
        ...
        if find_ir:
            ...
            for i in dfsummary.index:
                ...
                if step[0]:
                    #TODO use pd.loc[row,column] e.g. pd.loc[:,"charge_cap"] for col or pd.loc[(pd.["step"]==1),"x"]
                    ir = dfdata[(dfdata[c_txt] == cycle)  & (test.dfdata[s_txt] == step[0])][self.internal_resistance_txt]
                    # This will not work if there are more than one item in step                        
                    ir = ir.values[0]
                else:
                    ir = 0
                ...
                if step2[0]:
                    ...
                    ir2 = dfdata[(dfdata[c_txt] == cycle)  & (test.dfdata[s_txt] == step2[0])][self.internal_resistance_txt].values[0]
                else:
                    ir2 = 0
                ir_indexes.append(i)
                ir_values.append(ir)
...
def join_series(li):
    new_series = pd.concat(li, axis = 0)
    return new_series    
    
def get_voltage(d):
    f=arbindata()
    return d[f.voltage_txt]
   
   
def get_capacity(d):
    f=arbindata()
    discharge_cap = d[f.discharge_capacity_txt]
    charge_cap = d[f.charge_capacity_txt]
    if charge_cap.sum() > 0.0:
        return charge_cap
    else:
        return discharge_cap

def get_discharge_capacity(d,mass=None):
    f=arbindata()
    if mass:
        return d[f.discharge_capacity_txt]*1000000/mass
    else:
        return d[f.discharge_capacity_txt]
    
def get_charge_capacity(d,mass=None):
    f=arbindata()
    if mass:
        return d[f.charge_capacity_txt]*1000000/mass
    else:
        return d[f.charge_capacity_txt]

def get_current(d):
    f=arbindata()
    return d[f.current_txt]

def get_timestamp(d, in_minutes=False):
    f=arbindata()
    if in_minutes:
        return d[f.test_time_txt]/60.0
    else:
        return d[f.test_time_txt]
    
#----------------------tests---------------------------------------------------
    
def test_store_save():
    f = [[r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_01.res', 
             r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_02.res', 
             ]]
    mass = 1.854240252
    d = arbindata(f)
    d.loadres()
    d.set_mass(mass)
    d.make_summary()
    outfilename = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20130419_es018_02_eth"
    d.save_test(outfilename, test_number=0)
    print "saved the test"
    print "finnished"

def test_store_load():
    infilename = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20130419_es018_02_eth.h5"
    d = arbindata(infilename)
    d.loadres(filetype="h5")
    d.set_testnumber(0)
    timestamp, voltage, current, discharge_capacity, charge_capacity, test_name = d.quick_view()
    plot(charge_capacity, voltage)
    show()
    
    
    
def test1():
    f = r"C:\Scripting\MyFiles\ArbinDataViewer\RES-files\20130121_sl8_ETH_2.res"    
    d = arbindata(f)
    d.loadres() 
    d.set_mass(1.2)
    d.make_summary(find_ocv = False,
                    find_ir = True)
    d.exportcsv()
    d.get_ir()
    sys.exit()
    
    cl = d.tests[0].dfsummary["Cycle_Index"]
    ir = d.tests[0].dfsummary["Internal_Resistance"]
    #plot(cl,ir)
   # show()
    s = d.populate_step_dict(1)
    #sys.exit()
    v=d.find_step_numbers(step_type ="ir",only_one=False)
    print v
    sys.exit()
#    d.find_C_rates(v)
#    v=d._select_step(1,3)
    ocv = d._get_ocv(select_first = True,
                    select_last = False,
                    select_all = False)
    print "finnished"
    counter = 1
    ...
    for j in ocv:
        counter+=1
        if j and counter>intervall:
            counter = 0
            try:
                cycle = j["Cycle_Index"].values[0]
                labeltxt = "cycle %i step %i" % (cycle,j["Step_Index"].values[0])
            except:
                labeltxt = "None"
            plot(j["Step_Time"],j["Voltage"],"-o",label=labeltxt )
            v_min = j["Voltage"].min()
            v_max = j["Voltage"].max()
            dv = v_max-v_min
            print cycle, v_min, v_max, dv
    ...
    show()
    plot(cycles_list, v_min_list)
    plot(cycles_list, v_max_list)
    show()
    
#    step_dict={1:[1],2:[10],3:[10,11]}    
#    v2=d.select_steps(step_dict,append_df = True) 
#    plot(v2["Discharge_Capacity"])
#    show()
    #d.make_summary()
    #d.exportcsv()
    #d.report()
...
def test2():
    # select file and set the correct mass
    f = [r"L:\EnergyStorageMaterials\Data-backup\Arbin\20130416_gb001_01_eth_01.res",
         r"L:\EnergyStorageMaterials\Data-backup\Arbin\20130416_gb001_02_eth_01.res",
         #r"C:\Scripting\MyFiles\ArbinDataViewer\RES-files\20130219_Sl10_1_eth.res", # cannot find ocv properly for this file (e.g. missing 21-32)
         ]
    mass = [1.080, 
            1.086,
            #0.9,
    ...
    d.loadres() 
    d.set_mass(mass)
    timestamp, voltage, current, discharge_capacity, charge_capacity, test_name = d.quick_view(0)
    plot(charge_capacity, voltage)
    timestamp, voltage, current, discharge_capacity, charge_capacity, test_name = d.quick_view(1)
    plot(charge_capacity, voltage)

    show()
    
#    plot(data_point, volstart_time_1tage)
#    plot(data_point, current)
#    show()
    sys.exit()
    # time,voltage,current = d.quickview(set = 1, cycle_filter = all, step_filter = all)
    # plot()
    d.make_summary(find_ocv = True,
                   find_ir = True)
                   
    # need a function for appending columnt to summary file
    # or a function for selecting what to save (make a select_col, save_csv)
    d.exportcsv()
    
        
    print "loaded and saved files"

    print "now I will look at what is there..."
    print
    #d.report()
    
def test3():
    # Test to see if merging two files work
    
    f = [[r"L:\EnergyStorageMaterials\Data-backup\Arbin\20130305_es013_02_eth_01.res",
         r"L:\EnergyStorageMaterials\Data-backup\Arbin\20130305_es013_02_eth_02.res",
         r"L:\EnergyStorageMaterials\Data-backup\Arbin\20130305_es013_02_eth_03.res",
         ]]
    mass = [1.649, 
            #1.649, # works also with only one mass given (if same)
            ]
    mass = 1.649
    ...
    d.loadres() 
    d.set_mass(mass)
    #a = d._select_last(d.tests[0].dfdata)
    #print a
    #sys.exit()
    #print d.tests[0].dfdata
    #print d.tests[1].dfdata
    print "loaded the files - trying to merge"
    #d.merge()
    print "and now I make the summary"
    d.make_summary(find_ocv = False,
                   find_ir = False,
                   use_arbin_stat_file=True,)
    #print "loaded the files - trying to merge"
    ...
    #print d.tests[0].dfdata
    plot(d.tests[0].dfsummary["Discharge_Capacity(mAh/g)"],"r*")
    plot(d.tests[0].dfsummary["Charge_Capacity(mAh/g)"],"b*")
    show()
    
def test4():
    f = [[r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_01.res', 
             r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_02.res', 
             r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_03.res', 
             r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_04.res', 
             r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_05.res', 
             r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_06.res'],
             r'L:\\EnergyStorageMaterials\\Data-backup\\Arbin\\20130419_es018_02_eth_01.res']
    mass = [1.854240252,1.2]
    d = arbindata(f)
    d.loadres()
    d.set_mass(mass)
    number_of_different_sets = len(f)
    number_of_test = len(d.tests)
    print "# of sets",
    print number_of_different_sets
    print "# of tests",
...
def test5():
    # this test reads data from res-file, finds the discharge steps,
    # plots them and saves them to a csv file
    # kind-of-neat
    
    f = r"L:\EnergyStorageMaterials\Data-backup\Arbin\20130409_es013_07_eth_01.res"
    m = 1.1
    d = arbindata(f)
    d.loadres() 
    d.set_mass(m)
    d.make_summary(find_ocv = True,
                   find_ir = True)
    d.print_steps()
    number_of_tests = d.get_number_of_tests()  
    print "You have the following number of tests in your file:",
    print number_of_tests
    
    dataset = d.tests[0]    # selecting our experiment
    dataset = d.get_test(0) # same effect (remark, using python numbering, i.e. 0,1,2,...)
    dfdata = dataset.dfdata # selecting the DataFrame version of the data
    test_name = dataset.test_name # nice to get the name of the dataset / experiment as well
    sample_mass = dataset.mass # lets also get the sample mass (same as set in d.set_mass(m))
    
    # get point number
    data_point_txt = d.data_point_txt # pre-difened coulumheadings
    data_point = dfdata[data_point_txt]   
    # get timestamp
    time_txt = d.test_time_txt 
    timestamp = dfdata[time_txt]
    timestamp = timestamp/60.0
    # get voltage
    voltage_txt = d.voltage_txt
    voltage = dfdata[voltage_txt]
    # get current
    current_txt = d.current_txt
    current = dfdata[current_txt]
    # get charge capacity
    discharge_txt = d.discharge_capacity_txt
    discharge_capacity = dfdata[discharge_txt]*1000000/sample_mass # converting to mAh/g
    
    charge_txt = d.charge_capacity_txt
    charge_capacity = dfdata[charge_txt]*1000000/sample_mass
    
    #plot(charge_capacity,voltage)
    #show()
    
    # now I would like to plot individual voltage graphs
    discharge_steps = d.get_step_numbers(steptype = "discharge", allctypes=False,
                                          test_number=0)
    print discharge_steps 
    s = d.select_steps(discharge_steps,
                       test_number=0)
    # how many steps did I find?
    number_of_steps = len(s)
    print "found",
    ...
    for dischargestep in s[1:-1:10]:
        try:
            cycle_no = dischargestep[d.cycle_index_txt].iget(0)
            legendtxt = "cycle %i" % cycle_no
            print "plotting",
            print legendtxt
            discharge_capacity = dischargestep[discharge_txt]*1000000/sample_mass
            voltage = dischargestep[voltage_txt]
            plt.plot(discharge_capacity,voltage, label = legendtxt)
        except:
    ...
    # now it would be nice to be able to save this data to a file
    outfile = r"C:\Scripting\MyFiles\ArbinDataViewer\Outdata\outtest.csv"
    # I have tried several options for merging several pandas DataFrames into one
    # but not really found a good method
    # So, lets put it into lists instead and use csv and itertools

    out_data = []
    for step in s:
       # getting the cycle number
       cycle_no = step[d.cycle_index_txt].iget(0)
       header_x = "discharge_cap cycle_no %i" % cycle_no
       header_y = "voltage cycle_no %i" % cycle_no
       discharge_capacity = step[discharge_txt]*1000000/sample_mass
       discharge_capacity=discharge_capacity.tolist()
       voltage = step[voltage_txt].tolist()
       discharge_capacity.insert(0,header_x)
       voltage.insert(0,header_y)
       out_data.append(discharge_capacity)
       out_data.append(voltage)
...
def test6():   
    f = [r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140324_gb015_04_cc_01.res"]
    mass = 1.4327
    ...
    #d.merge()
    print "and now I make the summary"
    d.make_summary(find_ocv = True, find_ir = True)
    #print "loaded the files - trying to merge"
    #d.merge()
    ...
    plot(d.tests[0].dfsummary["Charge_Capacity(mAh/g)"],"b*")
    show()    
    outfilename = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20131016_es023_24_cc"
    #d.save_test(outfilename, test_number=0)
    print "OK"
        
        
def test7(f=None):
    # test to check new convenince functions (get_xxx)
    import time
    t0 = time.time()
    f2 = [r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20140324_gb015_04_cc.h5"]  
    f2 = [r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140324_gb015_04_cc_01.res"] 
    ...
    dt = t1-t0
    d.create_step_table()
    outname1 = r"C:\Scripting\MyFiles\dev_cellpy\outdata\step_table_test2.csv"
    d.tests[0].step_table.to_csv(outname1,sep=";")
    print "this took %f seconds" % (dt)
#    print "printing steps"
#    d.print_steps()
    print "printing cycle numbers"
    cn = d.get_cycle_numbers()
    print cn
    print "print mass"
    mass = d.get_mass()
    print mass
    print "get_cap"
    cycle = 2
#    dc, dcv = d.get_dcap(cycle)
#    plot(dc,dcv)
    cc, cv = d.get_ccap(cycle)
    #plot(cc,cv)
    x,y,px,py = d.get_cap(cycle,polarization=True,points = None)
    x2,y2,px2,py2 = d.get_cap(cycle,polarization=True,points = [100.0,500.0,1000.0,1500.0,1700.0])
    fig1 = figure()
    fig1.suptitle("capacity curves")
    ax1 = fig1.add_subplot(111)
    ax1.plot(x,y,'b-')
    ax1.set_ylabel("Voltage charge and discharge")
    ax1.set_xlabel("capacity (mAh/g)")
    ax1.grid()
    ax2 = ax1.twinx()
    ax2.plot(px,py,'r-')
    ax2.plot(px2,py2,'ro')
    ax2.set_ylabel("Voltage charge-discharge (hysteresis)", color = 'r')
    #show()  
    print
    print "extracting ocv"
    fig2 = figure()
    fig2.suptitle("ocv-curves")
    ax21 = fig2.add_subplot(111)
    cycles =  [1,2,3,5,10,20,30,40,50]
    for c in cycles:
        label = "cycle %i" % (c)
        stime,ocv = d.get_ocv(ocv_type='ocvrlx_up', cycle_number = c)
        if any(stime):
            ax21.plot(stime, ocv,label=label)
    legend()
    show()
...
def test_look_for_updated_files():
    """testing ways to look for updated files"""
    f = [r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140327_gb016_03_cc_01.res",
         r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140327_gb016_03_cc_02.res"]
    h5 = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20140124_es030_08_cc.h5"
    #f = r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140327_gb016_03_cc_01.res"
    
    d1 = arbindata(h5,verbose=True)
    d1.loadres() 
    
#    d2 = arbindata([f],verbose=True)
#    d2.loadres()
#    d2.set_mass(1.1)
#    #d2.merge() 
#    d2.make_summary(find_ocv = True,
#                   find_ir = True)
#
#    d2._create_infotable()
    
    res_file_names1 = d1.tests[0].raw_data_files
    sizes1          = d1.tests[0].raw_data_files_length #problem here
#    res_file_names2 = d2.tests[0].raw_data_files
#    sizes2          = d2.tests[0].raw_data_files_length
    
    print
    print "res files in hdf5:"
    for r in res_file_names1:
        print r

#    print "res files in raw-files:"
#    for r in res_file_names2:
#        print r
#        
    print "sizes"
    print sizes1
...
#    print
#    print "res files and sizes in hdf5:"
#    for f,z in zip(res_file_names1,sizes1):
#        print "%s: %i"
#    print  
#    print "res files and sizes in raw-files:"
#    for f,z in zip(res_file_names2,sizes2):
#        print "%s: %i"                     


def test9():
    # test to check generation of step_type dataframe 
    import time
    t0 = time.time()
    f = [r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140324_gb015_04_cc_01.res"]  
#    f = [r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140521_es039_03_cc_01.res",
#         r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140605_es039_04_cc_01.res"]
    # load the file
    d = arbindata(f)
    d.loadres() 
    d.set_mass(1.1)
    ...
    d.verbose = True
    d.create_step_table()
    outname1 = r"C:\Scripting\MyFiles\dev_cellpy\outdata\step_table_test.csv"
    outname2 = r"C:\Scripting\MyFiles\dev_cellpy\outdata\datatable_test.csv"
    d.tests[0].step_table.to_csv(outname1,sep=";")
    d.tests[0].dfdata.to_csv(outname2,sep=";")
    d.make_summary(find_ir = True, find_ocv = True)
    
    
    print "this took %f seconds" % (dt)
    print d._validata_step_table()
    #d.create_step_table()
#    print d._validata_step_table()
#    print d.step_table
    
    print "running get_steps"
    s = d.get_step_numbers('charge', test_number = 0)
    
#    outname1 = r"C:\Scripting\MyFiles\dev_cellpy\outdata\step_table_test.csv"
    
#    outname3 = r"C:\Scripting\MyFiles\dev_cellpy\outdata\datatable_test.h5"
#    sep = ";"
#    d.tests[0].step_table.to_csv(outname1,sep=sep)
#    d.make_summary(find_ocv = True,
#                   find_ir = True)
#    d.save_test(outname3)
    #d.tests[0].dfdata.to_csv(outname2,sep=sep)

    print "OK"   

def test_get_ccap():
    # test to check implementation of new get_cap etc 
    h5 = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20140124_es030_08_cc.h5"
    f = r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140124_es030_08_cc_01.res"
    ...
    d2.loadres()
    d2.set_mass(mass)
    d2.create_step_table()
    
    outname1 = r"C:\Scripting\MyFiles\dev_cellpy\outdata\step_table_test10.csv"
    outname2 = r"C:\Scripting\MyFiles\dev_cellpy\outdata\datatable_test10.csv"
    sep = ";"
    d2.tests[0].step_table.to_csv(outname1,sep=sep)
    d2.tests[0].dfdata.to_csv(outname2,sep=sep)
    ...
    print
    print "loading charge capacity"
    t,c = d1.get_ccap(cycle = 1)

    
def test_load_NiMH():
    # load NiMH data
    f = r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140702_mh005_p_01.res" #mh001, mh003, mh005 (thinnest)
    d = arbindata(verbose=True)
    d.loadres(f)
    d.set_mass(60)
    cycle = 1
    cc, cv = d.get_ccap(cycle)
    dc, dv = d.get_dcap(cycle)
    v = d.get_voltage(cycle)
    t = d.get_timestamp(cycle, in_minutes=True)
    i = d.get_current(cycle)
        
#    fig1 = figure()
#    fig1.suptitle("Capacity")
#    ax1 = fig1.add_subplot(111)
...
#    ax1.plot(cc,cv,'r-',label = "charge")
#    ax1.plot(dc,dv,'b-',label="discharge")
#    ax1.set_ylabel("Voltage")
#    show()
    
    fig2 = figure()
    fig2.suptitle("Capacity")
    ax3 = fig2.add_subplot(111)
    ax3.set_ylim([0.6,1.0])
    ax3.set_xlabel("discharge capacit (mAh/g)")
    ax3.set_ylabel("voltage vs HgO/Hg (V)")
    for j in d.get_cycle_numbers():
        l = "cycle %i" % j
        dc,dv = d.get_dcap(j)
        ax3.plot(dc,dv,'-', label =l)
    legend()
    show()
    
def test12():
    rdir =r"L:\EnergyStorageMaterials\Data-backup\Arbin"
    hdf5dir = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5"
    n ="20140224_es033_03_cc"
    d = arbindata()
    d.set_res_datadir(rdir)
    d.set_hdf5_datadir(hdf5dir)
    d.loadcell(n)
        

def check_existence_hdf5():
    # extract file-info from hdf5-file and compare with file-info from .res
    # USAGE
    #        resfiles = database.get_resfilenames(experiment_no)
    #        hdf5file = database.get_hdf5filename(experiment_no)
    #        d = arbindata()
    #        update_it = d.check_file_ids(hdf5file,resfiles,force=False)
    #        if update_it:
    #            d.loadres(filename = resfiles)
    #        else:
    #            d.loadres(filename = hdf5file)
    ...
    dt = time.time()-ti
    print "loading dbreader took %f sec" % (dt)
    #f_res = reader.get_filenames(srno, use_hdf5=False)
    ts = time.time()
    f_res = reader.get_resfilenames(srno)
    te = time.time()
    ts0=ts
    dt = te-ts
    print "get resfiles took %f sec" % dt
    #f_hdf5 = reader.get_filenames(srno)
    
    ts = time.time()
    f_hdf5 = reader.get_hdf5filename(srno)
    te = time.time()
    dt = te-ts
    print "get hdf5files took %f sec" % dt
    
    ts = time.time()
    m = reader.get_mass(srno)
    te = time.time()
    dt = te-ts
    print "get mass took %f sec" % dt
    ...
    print "checking if similar"
    d = arbindata(verbose=True)
    
    ts = time.time()
    similar = d.check_file_ids(f_hdf5,f_res, return_res=False)
    te = time.time()
    dt = te-ts
    print "checking similar took %f sec" % dt
    dt = te-ts0
    print "everything took %f sec" % dt
    
    # check_file_ids assumes f_res as list and f_hdf5 as txt, but has internal
    # checker
    if not similar:
        print "not similar"
        f = f_res
   ...
   # sys.exit()
    ...
    d.loadres(f)
    d.set_mass(m)
    cn = d.get_step_numbers("charge")
    dn = d.get_step_numbers("discharge")
    print cn
    cnd = d.find_C_rates(cn,silent=False)
    print
    print cnd
    print "---------------------------"
    print dn
    dnd = d.find_C_rates(dn,silent=False)
    print
    print dnd
...
def check_c_rates():
    ...
    #------------reading db--------------------------------------------------------
    reader = dbreader.reader()
    f = reader.get_filenames(srno, use_hdf5=False)
...
def check_speed(only_summary=False):
    filename = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20140520_es031_20_cc.h5"
    #filename =r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140520_es031_20_cc_01.res"
    d = arbindata(verbose=False, profile=True)
    d.loadres(filename)
    if only_summary is False:
        ...
        for cycle in list_of_cycles:
            ...
            if counter>100:
                break
            tx = time.time()
            t,c = d.get_ccap(cycle)
            txt += "cycle %i took %f sec\n" % (counter, time.time()-tx)
        print txt
        print
    print "-----------testing-make-summary---------------------------"
    t_0 =  time.time()
    d.make_summary(find_ocv = True, find_ir = False, find_end_voltage = False)
    txt= "make summary took %f sec\n" % (time.time()-t_0)
    print txt
    
def check_shifted_cap():     
    filename = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20141007_rr001_01_cc.h5"
    #filename =r"L:\EnergyStorageMaterials\Data-backup\Arbin\20140520_es031_20_cc_01.res"
    d = arbindata(verbose=False, profile=True)
    d.loadres(filename)
    print
    cyclenos = d.get_cycle_numbers()
    print cyclenos
    d.make_summary(find_ocv = False, find_ir = False)
    summarydata = d.get_summary()
   
    discharge_txt = d.summary_txt_discharge_cap
    charge_txt = d.summary_txt_charge_cap
    print "\ndischarge\n"
    print summarydata[discharge_txt]
    print "\ncharge\n"
    print summarydata[charge_txt]
    ...
    for i,cycle in enumerate(cyclenos):
        ...
        try:
            ...
            shifted_discharge_cap.append(D_n)
            shifted_charge_cap.append(D_n + C_n)
            dif.append(dd)
            d1+=dd
            ...
            print "C_n: %f, D_n: %f" % (C_n, D_n)
            print "C_n2: %f, D_n2: %f" % (C_n2, D_n2)
            print "ric_n: %f, ric_disc: %f, ric_sei: %f" % (ric_n, ric_dis_n, ric_sei_n)
            ...
            break
#    plot(shifted_discharge_cap, label = "shifted discharge capacity")
#    plot(shifted_charge_cap, label = "shifted charge capacity")
    plot(RIC_cum, label = "RIC (cum)")
    plot(RIC_disconnect_cum, label = "RIC disconn. (cum)")
    plot(RIC_sei_cum, label = "RIC EIS (cum)")
    xlabel("Cycle")
    ylabel("Capacity/RIC")
    legend()
    show()
    
def check_shifted_cap2():     
    filename = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20141007_rr001_01_cc.h5"
    d = arbindata(verbose=False, profile=True)
    d.loadres(filename)
    d.make_summary(find_ocv = False, find_ir = False)
    out = d.get_diagnostics_plots(scaled = True)
    plot(out["cycles"],out["RIC_cum"], label = "RIC (cum)")
    plot(out["cycles"],out["RIC_disconnect_cum"], label = "RIC disconn. (cum)")
    plot(out["cycles"],out["RIC_sei_cum"], label = "RIC EIS (cum)")
    xlabel("Cycle")
    ylabel("Capacity/RIC")
    legend()
    figure()
    plot(out["shifted_discharge_cap"], label = "shifted discharge capacity")
    plot(out["shifted_charge_cap"], label = "shifted charge capacity")
    legend()
    show()
    
def raw_data():
    filename = r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\20140715_is031_01_cc_01.res"
    outdir = r"C:\Scripting\MyFiles\dev_cellpy\test_raw_data"
    d = arbindata(verbose=True)
    d.loadres(filename)
    d.make_summary()
    d.create_step_table()
    ...
    c,v = d.get_cap()
    #data =d.tests[0]
    #c.to_csv(os.path.join(outdir, "test_c.csv"),sep=d.sep)
    for test in d.tests:
        print "\n**************************************"
        print "printing charge steps"
        print test.charge_steps
    print c
    
    
def test_export_cycles():
    filename = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20140328_es033_05_cc.h5"
    outdir = r"C:\Scripting\MyFiles\dev_cellpy\tests\out"
    d = arbindata(verbose=False)
    
    d.loadres(filename)
    d.set_mass(0.60)
    d.make_summary()
    d.create_step_table()
    d.exportcsv(outdir, cycles=True)
    
def test_bad_file():
    filename_bad = r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\20150506_nb001_01_cc_01.res"
    filename_big = r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\141107_FR_C_01_cc_01.res"
    filename_good = r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\20140715_is031_01_cc_01.res"
    outdir = r"C:\Scripting\MyFiles\dev_cellpy\tests\out"
    
    # TODO: check how big self.max_oneliners can be
    # TODO: use FileError as return value
    # FR file seems to work

    d = arbindata(verbose=True, fetch_onliners=False)
    d.loadres(filename_bad)
    d.set_mass(1.742)
    d.make_summary()
    d.create_step_table()
    d.exportcsv(outdir, cycles=True)
...
def test_bad_file2():
    filename_bad = r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\20140715_is031_01_cc_01.res"
    filename_good = r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\20140715_is031_02_cc_01.res"
    outdir = r"C:\Scripting\MyFiles\dev_cellpy\tests\out"
...
#    filenames = []
#    for srno in srnos:
#        masses.append(reader.get_mass(srno))
#        filenames.append(reader.get_cell_name(srno))
#    print masses
#    print filenames
#    sys.exit(0)
    print "*test_bad_file2: starting..."
    masses = [0.9238164298782164, 0.8601415467160196]
    filenames = [u'20140715_is031_01_cc', u'20140715_is031_02_cc']

    d = arbindata(verbose=True, fetch_onliners=False)
    d.loadcell(names = filenames, masses = masses)
    print "*test_bad_file2: loaded succesfully"
    d.make_summary()
    print "*test_bad_file2: made summary succesfully"
    d.exportcsv(outdir, cycles=True)   
    print "*test_bad_file2: exported csv succesfully"
    print "trying to save empty test:"
    d.save_test(os.path.join(outdir,"first_test.h5"), test_number = 0)
    print "trying to save good test:"
    d.save_test(os.path.join(outdir,"second_test.h5"), test_number = 1)
    print "*test_bad_file2: saved succesfully"
    
    
def test_load_hdf5_new():
    f = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20140814_nb004_04_cc.h5"
    f = r"C:\ExperimentalData\BatteryTestData\Arbin\HDF5\20140328_es033_05_cc.h5"
    # must update all hdf5 - files?
    # seems it cannot load the fidtable from the old hdf5-files after changing
    # name from arbinpy to cellpy
    d = arbindata(f,verbose = True)
    d.loadres()
    print "ok"
    
def test_save_load_hdf5():
    """testing ways to look for updated files"""
    f = [[r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\20141007_rr001_01_cc_01.res",
         r"I:\Org\ensys\EnergyStorageMaterials\Data-backup\Arbin\20141007_rr001_01_cc_02.res"]]
    h5 = r"C:\Scripting\MyFiles\dev_cellpy\tests\out\20141007_rr001_01_cc.h5"
    
    print "loading res-files"
    d_res = arbindata(f,verbose=True)
    d_res.ensure_step_table = True
    d_res.loadres()
    d_res.set_mass(1.441)
    #d_res.create_step_table()
    d_res.make_summary()
    d_res.save_test(filename=h5)
    print "ok -res"
    d_h5 = arbindata(h5, verbose=True)#e_header
    d_h5.loadres()
    print "ok -h5"
#    
#        
...
def just_load_res_and_mod(filename = None, mass = None):
    if filename is None:
        filename = r"C:\Scripting\MyFiles\dev_cellpy\tests\in\20140715_is031_01_cc_01.res"
    if mass is None:
        mass = 0.9238
    ...
    v = d_res.get_step_numbers("charge")
    print v
    
    print "looking at the step table"
    d_res.print_step_table()
    d_res._modify_cycle_number_using_cycle_step([1,4], 2)
    d_res._modify_cycle_number_using_cycle_step([1,5], 2)
    d_res.print_step_table()
    d_res._cap_mod_normal(capacity_modifier = "reset")
    #sys.exit()
    d_res.print_step_table()
    d_res._make_summary(use_arbin_stat_file=False,)
                        #capacity_modifier = "reset")
    d_res.exportcsv(os.path.dirname(filename), cycles=True) 
    
    print
    print "finding C-rates"
    d_res.find_C_rates(v,silent=False)
    
    print
    print "OK"
    
    
def just_load_res(filename = None, mass = None):
    if filename is None:
        filename = r"C:\Scripting\MyFiles\dev_cellpy\tests\in\20140715_is031_01_cc_01.res"
    if mass is None:
        mass = 0.9238
...
def test_new_step_table():
    filename = r"C:\Scripting\MyFiles\dev_cellpy\tests\in\20140702_mh005_p_01.res"
    mass = 0.9238
    d_res = setup_cellpy_instance()
    d_res.loadres(filename)
    d_res.set_mass(mass)
