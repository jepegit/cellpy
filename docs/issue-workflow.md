...
9. **After review** — if switched back, return to the PR branch before review fixes; merge when approved and `gh pr checks` is green. With `yolo`, close may `gh pr checks --watch` (budget: `checks_watch_minutes`, default 15) before merge, falling back to `--auto` only when the cap elapses.
...
**Result:** A batch report of merged/failed/not-reached issues.
