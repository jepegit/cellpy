...
- Plotting tests (`tests/test_plotutils_summary_plot.py`) run under `MPLBACKEND=Agg`
  in the Tier-1 essential subset, the Tier-2 full Linux suite with plotting
  extras (`ci.yml`), release, and the scheduled Tier-3 matrix
  (`ci-scheduled.yml`). Cases that need plotly/seaborn `skipif` when those
  extras are missing; install `[batch]` / `[all]` locally to exercise them.
